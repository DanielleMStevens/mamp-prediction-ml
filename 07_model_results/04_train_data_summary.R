...
train_data$Ligand.Length <- nchar(train_data$Sequence)
...
identity_calc <- function(label_ids, sequence_list, comparison){
  ...
  for (i in 1:length(label_ids)){
    for (j in 2:length(label_ids)){
      alignment <- pwalign::pid(pwalign::pairwiseAlignment(sequence_list[i], sequence_list[j], 
                    substitutionMatrix = "BLOSUM62", scoreOnly = FALSE, type = "global"))
...
# inr receptor
receptor_INR <- rbind(train_data %>% filter(Receptor == "INR"), train_data %>% filter(Receptor == "INR-like"))
...
# fls2 receptor
receptor_FLS2 <- train_data %>% filter(Receptor == "FLS2")
...
# peru receptor
receptor_PERU <- train_data %>% filter(Receptor == "PERU")
...
# mik2 receptor
receptor_MIK2 <- train_data %>% filter(Receptor == "MIK2")
...
# core receptor
receptor_CORE <- train_data %>% filter(Receptor == "CORE")
...
# efr receptor
receptor_EFR <- train_data %>% filter(Receptor == "EFR")
...
# fls3 receptor
receptor_FLS3 <- train_data %>% filter(Receptor == "FLS3")
...
# rlp42 receptor
receptor_RLP42 <- train_data %>% filter(Receptor == "RLP42")
...
# rlp23 receptor
receptor_RLP23 <- train_data %>% filter(Receptor == "RLP23")
...
# nut receptor
receptor_NUT <- train_data %>% filter(Receptor == "NUT")
...
ggsave(filename = "./04_Preprocessing_results/Train_plots/receptor_sequence_comparison_plot_train_only.pdf", 
...
# In11 epitope
epitope_In11 <- train_data %>% filter(Epitope == "In11")
...
# flg22 epitope
epitope_flg22 <- train_data %>% filter(Epitope == "flg22")
...
# Pep-25
epitope_Pep25 <- train_data %>% filter(Epitope == "pep-25")
...
# SCOOP
epitope_SCOOP <- train_data %>% filter(Epitope == "scoop")
...
# csp22
epitope_csp22 <- train_data %>% filter(Epitope == "csp22")
...
# elf18
epitope_elf18 <- train_data %>% filter(Epitope == "elf18")
...
# flgII-28
epitope_flgII28 <- train_data %>% filter(Epitope == "flgII-28")
...
# pg
epitope_peppg <- train_data %>% filter(Epitope == "pg")
...
# crip21  
epitope_crip21 <- train_data %>% filter(Epitope == "crip21")
...
# nlp 
epitope_nlp <- train_data %>% filter(Epitope == "nlp")
...
# screw
epitope_screw <- train_data %>% filter(Epitope == "screw")
...
ggsave(filename = "./04_Preprocessing_results/Train_plots/epitope_sequence_comparison_plot_train_only.pdf", 
...
# ridge plot
epitope_length_comparison_plot <- ggplot(train_data, aes(x = Ligand.Length, y = Epitope)) +
...
ggsave(filename = "./04_Preprocessing_results/Train_plots/epitope_length_comparison_plot_train_only.pdf", 
...
ggsave(filename = "./04_Preprocessing_results/Train_plots/receptor_sequence_comparison_plot_test_only.pdf", 
plot = receptor_sequence_comparison_plot, device = "pdf", dpi = 300, width = 2.5, height = 2.35)
...
ggsave(filename = "./04_Preprocessing_results/Train_plots/epitope_sequence_comparison_plot_test_only.pdf", 
plot = epitope_sequence_comparison_plot, device = "pdf", dpi = 300, width = 2.5, height = 2.55)
...
ggsave(filename = "./04_Preprocessing_results/Train_plots/epitope_length_comparison_plot_test_only.pdf", 
plot = epitope_length_comparison_plot, device = "pdf", dpi = 300, width = 2, height = 2)
