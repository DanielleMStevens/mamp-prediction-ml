...
class ESMWithReceptorAttnFilmModel(nn.Module):
    ...
    def forward(self, batch_x):
        ...
        peptide_embeddings = self.esm_model(**batch_peptide_x).last_hidden_state
        receptor_embeddings = self.esm_model(**batch_receptor_x).last_hidden_state
        embeddings = self.film(peptide_embeddings, receptor_embeddings, batch_peptide_x['attention_mask'], batch_receptor_x['attention_mask'])
        attention_mask = batch_peptide_x['attention_mask']
        mask_expanded = attention_mask.unsqueeze(-1).expand(embeddings.size())
        mean_pooled_output = (embeddings * mask_expanded).sum(dim=1) / mask_expanded.sum(dim=1)
        return self.net(mean_pooled_output) # (B, S)
